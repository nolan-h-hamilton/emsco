"""Python implementation of EMSCO"""
import argparse
import math
import random
import time
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Evolutionary Multi-stage Classifier Optimizer')
    ...
    parser.add_argument('--mutation_rate', type=float, default=0.075,
    ...
    parser.add_argument('--lr_iter', type=int,default=100,
      help="maximum number of gradient descent iterations for training LR models")
    parser.add_argument('--lr_solver',type=str,default="sag")
    parser.add_argument('--out',type=str, default = "emsco_output_{}.txt".format(date_stamp))
    ...
    val_columns = ['f' + str(i) for i in range(0,len(feature_times))]
    train_columns = ['f' + str(i) for i in range(0, len(feature_times))]
    test_columns = ['f' + str(i) for i in range(0, len(feature_times))]
    ...
    for i in range(iter_num):
        ...
        if i == args['iter_num']-1:
            ...
            output_file.write("feature_assignments\ttest_accuracy\ttest_coverage\ttest_cost\n")
            for sol in elite_population:
                elite_sol_test = classify_with_stages(clf, train_df, test_df,
                 sol.stage_list, feature_times, min_proba, solver_=args['lr_solver'],
                 lr_iter=args['lr_iter'])
                output_file.write(str_rep(sol) + "\t{}\t{}\t{}".format(round(elite_sol_test[1],4),
                    round(elite_sol_test[2],4),round(elite_sol_test[3],4)) + "\n")
            output_file.close()
